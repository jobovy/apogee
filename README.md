...
* Only stars in APOGEE's main sample (selected using a dereddened *J-Ks* > 0.5 color cut only) are included in the spectroscopic sample. See the function [*apogee.tools.read.mainIndx*](apogee/blob/master/apogee/tools/read.py#L345) for the precise sequence of targeting-flag cuts that define the main sample.
